...
int main()
{
	...
	//1   char* --- wchar_t*
	wchar_t *wch1 = tool.charTowchar(nullptr, "（1）腾盛 char* to wchar*");
	char * ch2 = tool.wcharTochar(nullptr, L"腾盛 wchar* to char*");
	wcout << wch1 << endl;
	cout << ch2 << endl;

	//2   CString ---- TCHAR*
	CString cstr81(tool.TCHARToCString(_T("（2）腾盛  TCHAR* to CString")));
	tcout << cstr81.GetString() << endl;
	CString s1 = _T("腾盛 CString to TCHAR*");
	TCHAR* ch82 = tool.CStringToTCHAR(nullptr, s1);
	tcout << ch82 << endl;
	
	//3 std::string/ std::wstring --- TCHAR*
	string str22 = ("（3）腾盛 string to TCHAR*");
	tcout << tool.stringToTCHAR(str22) << endl;
	wstring wstr22 = _T("腾盛 wstring to TCHAR*");
	tcout << tool.wtringToTCHAR(wstr22)<<endl;
	

	//4 TCHAR* 到std::string/ std::wstring的转换；
	TCHAR* test1 = _T("（4）腾盛 TCHAR* TO string");
	TCHAR* test2 = _T("腾盛 TCHAR* TO wstring");
	...
	//5  CStringA --- CString
	CString cstr1 = _T("（5）腾盛 CStringToCStringA 测试1");
	CStringA& cstrA1 = tool.CStringToCStringA(cstr1);
	cout << cstrA1.GetString() << endl;

	CStringA cstrA2 = "腾盛 CStringAToCString 测试2";
